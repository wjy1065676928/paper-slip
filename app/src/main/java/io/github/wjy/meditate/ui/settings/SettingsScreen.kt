...
fun SettingsScreen(
    ...
    // 统一遮罩可见性判断
    val isOverlayVisible = showScanOverlay || showWebDavDialog || (restorePreview != null)
        ...
        BlurBackground(
            ...
            Scaffold(
                ...
                LazyColumn(
                    ...
                    item { Text("数据管理", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp)) }
                    item { ListItem(headlineContent = { Text("WebDAV 同步") }, supportingContent = { Text(webDavConfig.url.ifBlank { "未配置" }) }, modifier = Modifier.clickable { showWebDavDialog = true }) }
                    if (webDavConfig.url.isNotBlank()) {
                        item { ListItem(headlineContent = { Text("立即同步到云端") }, supportingContent = { Text("上传本地数据库到 WebDAV") }, modifier = Modifier.clickable { viewModel.syncNow() }) }
                        item { ListItem(headlineContent = { Text("从云端恢复") }, supportingContent = { Text("从 WebDAV 下载并预览备份") }, modifier = Modifier.clickable { viewModel.downloadForRestore() }) }
                    }
                    item { HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp)) }
                    item { Text("分享", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp)) }
                    item { ListItem(headlineContent = { Text("扫码") }, supportingContent = { Text("扫描对方二维码预览记录") }, modifier = Modifier.clickable { showScanOverlay = true }) }
                    item { ListItem(headlineContent = { Text("全部分享") }, supportingContent = { Text("向对方展示所有") }, modifier = Modifier.clickable {
                        scope.launch {
                            try {
                                ...
                                } else { Toast.makeText(context, "数据库文件不存在", Toast.LENGTH_SHORT).show() }
                            } catch (e: Exception) { Toast.makeText(context, "分享失败: ${e.message}", Toast.LENGTH_SHORT).show() }
                        }
                    }) }
                    item { HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp)) }
                    item { Text("模糊", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp)) }
                    item { ListItem(headlineContent = { Text("模糊开关") }, supportingContent = { Text("在弹窗出现时模糊背景") }, trailingContent = { Switch(checked = blurEnabled, onCheckedChange = { viewModel.setBlurEnabled(it) }) }) }
                    item { ListItem(headlineContent = { Text("模糊强度") }, supportingContent = { Column { Slider(value = blurIntensity, onValueChange = { viewModel.setBlurIntensity(it) }, valueRange = 1f..25f, steps = 24); Text("当前强度: ${blurIntensity.toInt()}", style = MaterialTheme.typography.bodySmall) } }) }
                    item { HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp)) }
                    item { Text("Debug 调试", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.error, modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp)) }
                    item { ListItem(headlineContent = { Text("生成测试数据") }, supportingContent = { Text("点击生成 10 条随机日记记录") }, modifier = Modifier.clickable { viewModel.generateDebugEntries(10) }) }
                    item { ListItem(headlineContent = { Text("清空数据库") }, supportingContent = { Text("永久删除所有记录，请谨慎操作") }, modifier = Modifier.clickable { viewModel.clearAllEntries() }) }
                }
            }
        }
    }
